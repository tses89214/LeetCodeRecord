/*
 ...
 the total number of requested confirmation messages. The confirmation rate of a user that did not request any confirmation messages is 0. Round the confirmation rate to two decimal places.
...
SELECT 
    ...
    IFNULL(
        round(
        IFNULL(confirmation_count.confirm,0) / IFNULL(total_count.total,0)
        ,2)
    ,0) as confirmation_rate
FROM user_ids
LEFT JOIN total_count
ON user_ids.user_id = total_count.user_id
LEFT JOIN confirmation_count
ON user_ids.user_id = confirmation_count.user_id